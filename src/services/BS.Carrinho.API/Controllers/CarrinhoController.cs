...
namespace BS.Carrinho.API.Controllers
{
    ...
    public class CarrinhoController : MainController
    {
        ...
        {
            var produtoItemExistente = carrinho.ItemExistente(item);
        ...
        private async Task<CarrinhoItem> ObterItemCarrinhoValidado(Guid produtoId, CarrinhoCliente carrinho, CarrinhoItem item = null)
        {
            ...
            var itemCarrinho = await _context.CarrinhoItens
                .FirstOrDefaultAsync(i => i.CarrinhoId == carrinho.Id && i.ProdutoId == item.ProdutoId);

            if (itemCarrinho == null || !carrinho.ItemExistente(item))
            {
